...
class AuthAppXigua extends AuthApp
{
    const URL_CONNECT = 'https://open-api.ixigua.com/oauth/connect';

    const URL_TOKEN = 'https://open-api.ixigua.com/oauth/access_token/';
    const URL_REFRESH_TOKEN = 'https://open-api.ixigua.com/oauth/refresh_token/';
    const URL_REFRESH_REFRESH_TOKEN = 'https://open-api.ixigua.com/oauth/renew_refresh_token/';

    const URL_USER_INFO = 'https://open-api.ixigua.com/oauth/userinfo/';
    //西瓜视频用抖音的域名
    const URL_VIDEO_LIST = 'https://open.douyin.com/xigua/video/list/';
    const URL_VIDEO_DATA = 'https://open.douyin.com/xigua/video/data/';
    const URL_VIDEO_UPLOAD = 'https://open.douyin.com/xigua/video/upload/';
    const URL_VIDEO_UPLOAD_PART_INIT = 'https://open.douyin.com/xigua/video/part/init/';
    const URL_VIDEO_UPLOAD_PART_UPLOAD = 'https://open.douyin.com/xigua/video/part/upload/';
    const URL_VIDEO_UPLOAD_PART_COMPLETE = 'https://open.douyin.com/xigua/video/part/complete/';
    const URL_VIDEO_CREATE = 'https://open.douyin.com/xigua/video/create/';
...
//        print_r($response);
    ...
    static public function checkResponse($response){
        ...
        if(empty($arr)) throw new \Exception('西瓜接口返回错误',20000);
        $data = $arr['data'];
        if(!isset($data['error_code'])) throw new \Exception('西瓜接口返回错误',20000);
        if($data['error_code']!=0){
            Yii::error($response);
            throw new \Exception('西瓜接口返回错误:'.$data['error_code'].":".$data['description'],20000);
        }
        return $data;
    }
